...
func newArticle(db *gorm.DB, opts ...gen.DOOption) article {
	...
	_article.User = articleBelongsToUser{
		...
		UserInfo: struct {
			...
			RelationField: field.NewRelation("User.UserInfo", "model.UserInfo"),
		},
		Articles: struct {
			field.RelationField
			User struct {
			...
			RelationField: field.NewRelation("User.Articles", "model.Article"),
			User: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("User.Articles.User", "model.User"),
			},
			Tags: struct {
				field.RelationField
				Articles struct {
				...
				RelationField: field.NewRelation("User.Articles.Tags", "model.Tag"),
				Articles: struct {
					field.RelationField
				}{
					RelationField: field.NewRelation("User.Articles.Tags.Articles", "model.Article"),
				},
			},
		},
	}
...
func (a articleBelongsToUser) Model(m *model.Article) *articleBelongsToUserTx {
	return &articleBelongsToUserTx{a.db.Model(m).Association(a.Name())}
}

type articleBelongsToUserTx struct{ tx *gorm.Association }

func (a articleBelongsToUserTx) Find() (result *model.User, err error) {
	return result, a.tx.Find(&result)
}

func (a articleBelongsToUserTx) Append(values ...*model.User) (err error) {
...
func (a articleBelongsToUserTx) Replace(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a articleBelongsToUserTx) Delete(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
